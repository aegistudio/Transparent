...
void main() {
	...
	for(current = 0; current < maxLights; current ++) {
		...
		if(_shadow[current] == 0.0)
			...
			ambients += gl_LightSource[current].ambient;
			diffuses += gl_LightSource[current].diffuse * normal_light * _shadow[current];
			speculars += gl_LightSource[current].specular * specularFactor * _shadow[current];
		}	
		else {
			...
			ambients += gl_LightSource[current].ambient;
			diffuses += gl_LightSource[current].diffuse * normal_light * attenuation * _shadow[current];
			speculars += gl_LightSource[current].specular * specularFactor * attenuation * _shadow[current];
		}
	}
